...
CChartAxis::CChartAxis() 
 : m_pParentCtrl(NULL), m_bIsHorizontal(true), m_bIsInverted(false), 
   ...
   m_MaxValue(0), m_MinValue(0), m_GapValue(0), m_UnzoomMax(0), m_UnzoomMin(0), 
...
void CChartAxis::SetGap(double gap)
{
	m_GapValue = gap;
}
...
bool CChartAxis::RefreshAutoAxis()
{
	...
	if ( (SeriesMax!=m_MaxValue) || (SeriesMin!=m_MinValue) )
		SetMinMax(SeriesMin-m_GapValue,SeriesMax+m_GapValue);
